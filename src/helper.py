...
def api_model_inference(model, raster_patches, raster_patches_mask, raster_patches_metadata, data_transform, kernel, stride, device=torch.device('cpu'), no_data=-9999):
    pred_patches = model(
        data_transform(
            raster_patches.squeeze()
        ).to(device)
    )
    ...
    pred_union = torch.nan_to_num(
        stt.unary_union(
            pred_patches.unsqueeze(0),
            C=raster_patches_metadata['C'],
            H=raster_patches_metadata['H'],
            W=raster_patches_metadata['W'],
            kernel_size=kernel,
            stride=stride
        ),
        nan=no_data
    )
    pred_union[~raster_patches_mask] = np.nan

    return pred_union


def api_visualize_model_results(raster_pred_band, fig, ax, cmap):
    # vmin, vmax = 170, 215
    vmin, vmax = np.nanmin(raster_pred_band.squeeze()), np.nanmax(raster_pred_band.squeeze())

    divider = make_axes_locatable(ax)
    cax_pred = divider.append_axes('right', size='10%', pad=0.05)

    map_pred = ax.imshow(raster_pred_band.squeeze(), cmap=cmap, vmin=vmin, vmax=vmax)
